...
module.exports = {
    ...
    data: new SlashCommandBuilder()
        ...
        .addStringOption(option =>
            option.setName('rejected-reason')
                ...
                .setRequired(false))
        .addStringOption(option =>
            option.setName('steam-url')
                .setDescription('VP user\'s steam profile url')
                .setRequired(false)),
    ...
    async execute(interaction, client) {
        ...
        if (options === 'form-accepted') {
            ...
            const steam = await interaction.options.getString("steam-url") || 'no steam URL';

            if (!steam.includes('https://steamcommunity.com/')) {
                return await interaction.editReply({ content: "**ERROR** Invalid or Missing steam URL", ephemeral: true });
            }

            const vpChan = client.channels.cache.get(client.visa.VISA.VISACHAN);

            await vpChan.send({
                content: `**Application No: ${appNo.toString()}**, <@${user.id}> **Congratulations! Welcome To Iconic Roleplay. Happy Roleplay!**\n${steam}`

            }).then(async (msg) => {

                var msgLink = msg.url;

                await Roles(user, 'add', client.visa.ACCEPTED.ROLEID).then(async () => {
